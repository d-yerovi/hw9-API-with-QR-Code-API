...
@router.post("/qr-codes/", response_model=QRCodeResponse, status_code=status.HTTP_201_CREATED, tags=["QR Codes"]) ### ERROR "/qr-codes/" and status.HTTP_200_OK
async def create_qr_code(request: QRCodeRequest, token: str = Depends(oauth2_scheme)):
    ...
    if qr_code_full_path.exists():
        ...
        return JSONResponse(
            status_code=status.HTTP_409_CONFLICT, ### ERROR HTTP_200_OK
...
@router.delete("/qr-codes/{qr_filename}", status_code=status.HTTP_204_NO_CONTENT, tags=["QR Codes"]) ### "qr_fileame" and ERROR status.HTTP_200_OK
